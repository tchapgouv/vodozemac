...
impl DoubleRatchet {
    pub fn next_message_key<R: RngCore + CryptoRng>(&mut self, rng: &mut R) -> Option<MessageKey> {
    ...
    pub fn encrypt_truncated_mac<R: RngCore + CryptoRng>(
        &mut self,
        plaintext: &[u8],
        rng: &mut R,
    ) -> Result<Message, EncryptionError> {
...
impl InactiveDoubleRatchet {
    fn activate<R: RngCore + CryptoRng>(&self, rng: &mut R) -> Option<ActiveDoubleRatchet> {
